    ...
    Highlighter.prototype.highlight = function () {
            ...
            if (wordMatches) {
                ...
                $.each(wordMatches, function (index, match) {
                    ...
                    if (evaluated.indexOf(match) === -1) {
                        text = text.replace(new RegExp(match, 'g'), '<mark style="background-color:#FAFAFA;color:' + color + ';">$&</mark>', 'g');
        ...
        $.each(this.settings.ranges, function (i, range) {
            if (range.start < text.length) {
                ...
                var mark = '<mark style="background-color:#FAFAFA;color:' + range.color + ';"';
                if (range.class != null) {
